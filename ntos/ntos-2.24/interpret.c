...
{
  sprintf(buff, "NTOS %s", PATCH_LEVEL);
}
...
#endif
{
  ...
  if (v->type == T_STRING) {
    ...
    if(v->u.refed->ref != UINT_MAX) --v->u.refed->ref;
...
void pop_control_stack()
{
  ...
  if ((csp->framekind & FRAME_MASK) == FRAME_FUNCTION){
    DTRACE_PROBE3(ntos, lpc__return, current_object->obname, current_prog->function_table[csp->fr.table_index].funcname, current_prog->filename);
  }
#endif
...
#endif
   DTRACE_PROBE3(ntos, lpc__entry, current_object->obname, current_prog->function_table[findex].funcname, current_prog->filename);
...
#endif
  DTRACE_PROBE3(ntos, lpc__entry, csp->ob->obname, current_prog->function_table[findex].funcname, current_prog->filename);
    ...
    case F_ADD_EQ:
      ...
      case T_BUFFER:
        if (sp->type == T_BUFFER) {
          ...
          b = allocate_buffer(lval->u.buf->size + SVALUE_STRLEN(sp));
          memcpy(b->item, lval->u.buf->item, lval->u.buf->size);
          memcpy(b->item + lval->u.buf->size, sp->u.string, SVALUE_STRLEN(sp));
          free_buffer(lval->u.buf);
...
#ifdef CACHE_STATS
unsigned long apply_low_call_others = 0;
unsigned long apply_low_cache_hits = 0;
unsigned long apply_low_slots_used = 0;
unsigned long apply_low_collisions = 0;
#endif
...
int apply_low (const char * fun, object_t * ob, int num_arg)
{
  ...
  static long cache_mask = APPLY_CACHE_SIZE - 1;
...
#endif
        DTRACE_PROBE3(ntos, lpc__entry, ob->obname, fun, current_prog->filename);
	previous_ob = current_object;
        current_object = ob;
...
void get_line_number_info (const char ** ret_file, int * ret_line)
{
  ...
  if (!(*ret_file) && current_prog) // Added by Lonely.
