...
char *translate(iconv_t tr, const char *mes, int inlen, int *outlen){
    ...
    while(len)
    {
        if( iconv(tr, (const char **)&tmp, &len, &tmp2, &len2) == -1 )
        {
		if(errno == E2BIG)
		{
		  errno = 0;
		...
		/*
			...
			error("iconv cannot convert(EILSEQ)\n");
		else if(errno == EINVAL)
			error("iconv cannot convert(EINVAL)\n");
		else
			error("iconv cannot convert(Unknown)\n");
		*/
	}
