...
void fatal (const char *fmt, ...)
{
  ...
  case 0:
    ...
    va_end(args);
    debug_message("******** FATAL ERROR: %s\nNTOS driver attempting to exit gracefully.\n", msg_buf);
    if (current_file)
...
void error_handler (char * err)
{
...
#endif
    {
        ...
        if (current_heart_beat) {
            static char hb_message[] = "NTOS driver tells you: You have no heart beat!\n";
...
void error (const char * const fmt, ...)
{
    ...
    va_end(args);
    err_buf[0] = '*';           /* all system errors get a * at the start */
    DTRACE_PROBE1(ntos, error, (char *)err_buf);
    error_handler(err_buf);
}
...
void shutdownMudOS (int exit_code)
{
    ...
    shout_string("NTOS driver shouts: shutting down immediately.\n");
#ifdef PACKAGE_MUDLIB_STATS
    save_stat_files();
#endif
#ifdef PACKAGE_ASYNC
...
void slow_shut_down (int minutes)
{
    ...
    if (!MASTER_APPROVED(amo))
    {
        ...
        shout_string("NTOS driver shouts: Out of memory.\n");
        restore_command_giver();
