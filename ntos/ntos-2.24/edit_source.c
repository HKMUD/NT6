...
static void handle_configure() {
    ...
    fprintf(yyout, "#define SIZEOF_INT %ld\n", (long)sizeof(int));
    fprintf(yyout, "#define SIZEOF_PTR %ld\n", (long)sizeof(char *));
    fprintf(yyout, "#define SIZEOF_SHORT %ld\n", (long)sizeof(short));
    fprintf(yyout, "#define SIZEOF_FLOAT %ld\n", (long)sizeof(float));
    fprintf(yyout, "#define SIZEOF_LONG %ld\n", (long)sizeof(long));
    ...
    */
    ...
    if (lookup_define("USE_ICONV")) {
        if( !(check_library("-liconv") || check_library("-L/usr/local/lib -liconv")) )
            fprintf(stderr, "Cannot find liconv.a, compilation is going to fail miserably\n");
    }
    */
