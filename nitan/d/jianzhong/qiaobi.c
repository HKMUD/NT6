...
void check_dodge(object me,object room)
{
        if( (query("combat_exp", me)<50000) || (me->query_dex()<30) )
        {
                ...
                message("vision",query("name", me)+"突然从峭壁上摔了下来。\n",environment(me),me);
                if( (int)me->query_skill("dodge",1) > 50 )
                        me->unconcious();
                else 
                {
                        set_temp("last_damage_from", "从悬崖摔下来", me);
                        me->die();
                }
        } else
        if( (query("combat_exp", me)<100000) || (me->query_dex()<32) )
        {
                ...
                me->move(__DIR__"qiaobi");
                set("climb_trigger", 0, room);
                message("vision",query("name", me)+"垂头丧气从峭壁上爬了下来。\n",environment(me),me);
...
void climb_finish(object me,object room)
{
        ...
        message("vision",query("name", me)+"从峭壁下爬了上来。\n",environment(me),me);
}
int do_wa(string arg)
{
        ...
        message("vison",query("name", this_player())+"把峭壁上较低的青苔都用手挖去了。\n",environment(this_player()),this_player());
