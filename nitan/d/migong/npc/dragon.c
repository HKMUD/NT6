...
void create()
{
        ...
        set("age", 8888);
        set("long", @LONG
这是一只代表月（Moon），掌管着Vampire Land，其能力为吸入垂耳兔兽并分解他们的资料使自己拥有不死之身的魔族。
不受任何挪移、致命一击、致晕攻击、忙乱的效果影响。
LONG );
...
void death_msg()
{
        command("chat 人间居然有如此厉害的角色，看来我魔族在人间无立身之地了！");
        message_vision("$N长啸一声，响彻天地，然后身体慢慢变得透明，消失在无尽魔气之中。\n", this_object());
        CHANNEL_D->do_channel(this_object(), "rumor",
                        "听说" + name() + HIM "在人间受辱后，回到了魔族，从此不再出没人间。" NOR);
}
...
void random_move()
{
        if (time() - query_temp("born_time") > 1800)
        {
                message_vision("$N长啸一声，响彻天地，然后身体慢慢变得透明，消失在无尽魔气之中。\n", this_object());
                CHANNEL_D->do_channel(this_object(), "rumor",
                        "听说" + name() + HIM "在人间走了一遭后，百无聊奈，又返回了魔界。" NOR);
