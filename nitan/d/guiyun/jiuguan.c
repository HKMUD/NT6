...
int do_eat(string arg)
{
        ...
        message_vision("$N端起一"+query_temp("cai_"+i+"/unit", me)+query_temp("cai_"+i+"/name", me)+"，吃了两口。\n",me);
        remaining=query_temp("cai_"+i+"/remaining", me);
        remaining--;
        if (remaining == 0)
        {
                message_vision("$N将一"+query_temp("cai_"+i+"/unit", me)+query_temp("cai_"+i+"/name", me)+"吃得一干二净。\n",me);
                for (; i < count - 1; i++)
                {
...
int do_drink(string arg)
{
        ...
        message_vision("$N端起一"+query_temp("cai_"+i+"/unit", me)+query_temp("cai_"+i+"/name", me)+"，喝了两口。\n",me);
        remaining=query_temp("cai_"+i+"/remaining", me);
        remaining--;
        if (remaining == 0)
        {
                message_vision("$N将一"+query_temp("cai_"+i+"/unit", me)+query_temp("cai_"+i+"/name", me)+"喝得一干二净。\n",me);
                for (; i < count - 1; i++)
                {
                        set_temp("cai_"+i+"/name",query_temp("cai_"+(i+1,  me)+"/name"), me);
                        set_temp("cai_"+i+"/short",query_temp("cai_"+(i+1,  me)+"/short"), me);
