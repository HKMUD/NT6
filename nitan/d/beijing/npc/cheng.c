...
string ask_me1()
{
        if( query_temp("marks/鸡2", this_player()) )
        {
                delete_temp("marks/鸡2", this_player());
                set_temp("marks/鸡3", 1, this_player());
                return "哦，是老谭让你来的。到了这里就别见外了。你找我有什么事吗？\n";
        }
        else if( query_temp("marks/鸡3", this_player()) )
...
string ask_me2()
{
        if( query_temp("marks/鸡3", this_player()) )
        {
...
int accept_object(object who, object ob)
{
    ...
    if( (query("money_id", ob)) && (ob->value() == 50000 )
          && (query_temp("marks/鸡3", who)) )
        {
        item = new("/d/city/obj/jiaohuaji");
        item->move(who);
        delete_temp("marks/鸡3", who);
        return 1;
    }
