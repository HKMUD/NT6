...
string *MjC = ({"",
        ...
        "Ò»Ë÷","¶þË÷","ÈýË÷","ËÄË÷","ÎåË÷","ÁùË÷","ÆßË÷","°ËË÷","¾ÅË÷",
...
void create()
{
        set_name("ÂéÍ¯",({"ma tong","matong","kid"}));
        set_weight(1);
        set("long","Ò»¸öÁ³ÉÏÉúÁËÒ»Ð©Ð¡Âé×ÓµÄÐ¡º¢¡£");
        set("inquiry",([
                "mj": (: to_say_play :),
                "Âé½«": (: to_say_play :),
                "Íæ·¨": (: to_say_play :),
...
int do_sit()
{
        ...
        {
                if(!Playing) message_vision("$N×øÔÚµØÉÏµÅ×ÅÍÈ¶ùË£ÆÃ£¬±ñ×Å×ì¶ù¿Þ¿ÞÌäÌä¡£\n",me);
                else message_vision("$N´êÁË´êÊÖ£¬ºÇÁËÒ»¿ÚÆø£º½ñÌì×ÔÃþ"+(random(12)+1)+"´Î¾ÍºÃ¡£\n",me);
                return 1;
        }
        if (Playing)
        {
                if (random(1))
                        ...
                        "Ö¸×Å"+me->name()+"ËµµÀ£ºÈË¼ÒÕýÍæ×ÅÄØ£¬Äúµ½ÅÔ±ß×ø»á¶ù°É¡£\n",me);
                else
                        message_vision("$N×øÔÚµØÉÏµÅÍÈ¶ùË£ÆÃ£¬±ñ×Å×ì¶ù¿Þ¿ÞÌäÌäµØÈÂÈÂ£ºÎÒÒ²ÒªÍæÀ²¡«¡«¡£\n",me);
                return 1;
        }
...
int do_da(string str,object me)
{
        ...
        if (!Playing)
        {
                ...
                command("say ÅÆ¾Ö»¹Ã»¿ªÊ¼ÄØ£¬Äú¿É±ðÄÃÅÆÂÒ¶ª£¡");
                return 1;
        }
        ...
        if (!Touched)
        {
                ...
                command("say Äú»¹Ã»ÃþÅÆÄÄ£¡");
                return 1;
        }
        if (Goned==5)
        {
                message_vision("$NËµµÀ£ºÕâ°ÑÑ½£¬ÊÇ"+HIR+"Îå»¨ºÏÅÆ"+NOR+"£¬ÖØÐÂ¿ªÊ¼°É¡£\n",this_object());
                Playing=0;
                Count++;
                if (Play==4)
                {
        ...
        if (check==0) return notify_fail("ÄúÃ»"+do_check_num(str,0)+"ÕâÕÅÅÆÑ½¡£\n");
        ...
        NowPlayer++;
        NowPlayer%=Play;
        message_vision("$NËµµÀ£º¸Ã"+HIY+NowPlayerId[NowPlayer]+NOR+"ÃþÅÆÁË¡£\n",this_object());
        if (Auto_Tin)
        {
        ...
        if (count==(end-16-Goned))
        {
                Playing=0;
                Count++;
                message_vision("$NËµµÀ£ºÕâ°ÑÑ½£¬ÊÇ"+HIY+"Á÷¾Ö"+NOR+"£¬ÖØÐÂ¿ªÊ¼°É¡£\n",this_object());
...
int do_eat(string str)
{
        ...
        {
                command("stare"+query("id", me));
                command("say Õâ°ÑÑ½£¬Äú¾Í±ßÉÏ¿´×Å°É¡£");
                return 1;
        }
        if (!Playing)
        {
        ...
        {
                command("hammer"+query("id", me));
                command("say Ã»ÂÖµ½ÄúÄØ£¬ÓÆ×Åµã¶ùÀ´°É¡£");
                return 1;
        }
        ...
        if (do_check_eat(num1,num2,LastDump)==0)
        {
                ...
                command("say ÇÆÄúÄÇÑÛÉñ¶ù£¬ÕâÑùÒ²ÄÜ³ÔÑ½¡£");
                return 1;
        }
        if (num1==num2)
        {
        ...
        if (LastDumpMan==0)
                return notify_fail("Ã»ÓÐ["+num1+"]ÕâÖÖ´úÂë¡£\n");
        LastDumpMan=do_check_Mj(MjE,num2);
        if (LastDumpMan==0)
                return notify_fail("Ã»ÓÐ["+num2+"]ÕâÖÖ´úÂë¡£\n");
        LastDumpMan=do_check_Mj(who["Mj"],num1);
        if (LastDumpMan==0)
                return notify_fail("ÄúÃ»"+do_check_num(num1,0)+"ÕâÕÅÅÆÑ½¡£\n");
        LastDumpMan=do_check_Mj(who["Mj"],num2);
        if (LastDumpMan==0)
                return notify_fail("ÄúÃ»"+do_check_num(num2,0)+"ÕâÕÅÅÆÑ½¡£\n");
        message_vision("\n$NÅÄ³ö"+HIG+do_check_num(num1,0)+NOR+
                "ºÍ"+HIY+do_check_num(num2,0)+NOR+
...
int do_gang(string str,object me)
{
        ...
        {
                command("stare"+query("id", me));
                command("say ÄÇÕÅÅÆ¿ÉÊÇÄú×Ô¸ö¶ù´òµÄ¡£");
                return 1;
        }
        if( Touched && NowPlayerId[NowPlayer] != query("id", me) )
        {
                command("hammer"+query("id", me));
                command("say ±ðÈË¶¼»¹Ã»´òÅÆÄØ£¬Äú¸ÜÊ²Ã´Ñ½¡£");
                return 1;
        }
        if (!str) str=LastDump;
        ...
        if (i==1)
        {
                if (!Touched) return notify_fail("Äú»¹Ã»ÃþÅÆÄØ¡£\n");
                i=do_check_Mj(who["OutPeng"],str);
                if (!i) return notify_fail("ÄúÖ»ÓÐÒ»ÕÅ²»ÄÜ¸Ü¡£\n");
                if (NO_GANG) return notify_fail("ÕâÑù¿É¸Ü²»ÁË¡£\n");
                message_vision("\n$NËµµÀ£º"+HIB+"Ä¾¹¤"+NOR+"¡«¡«¡«\n\n",me);
                who["Mj"]=do_delete_Mj(who["Mj"],str,1);
        ...
        if (i < 3)
                return notify_fail("ÄãÊÖÖÐÒªÓÐÈýÕÅ"+do_check_num(str,0)+"²ÅÄÜ¸Ü¡£\n");
        if (!Touched)
        {
                if (str!=LastDump) return notify_fail("ÕâÑù¿É¸Ü²»ÁË¡£\n");
        }
        else if (i < 4)
                        return notify_fail("ÄãÊÖÖÐÒªÓÐËÄÕÅ"+do_check_num(str,0)+"²ÅÄÜ°µ¸Ü¡£\n");
        for (i=0;i<Play;i++)
        {
        ...
        {
                NO_HU=0;
                message_vision("\n$NÁ³ÉÏ²»½ûÂ¶³öÐ°¶ñµÄÐ¦ÈÝ£ºhia hia hia ...\n\n",me);
                who["Mj"]=do_delete_Mj(who["Mj"],str,1);
...
varargs int do_win(string str,object me)
{
        ...
        {
                command("stare"+query("id", me));
                command("say Õâ°ÑÑ½£¬Äú¾ÍÔÚ±ßÉÏÇÆ×Å°É¡£");
                return 1;
        }
        if (!Playing)
        {
        ...
        if( NO_HU || LastDumpId == query("id", me) )
                return notify_fail("ÕâÑù¿É²»ÄÜºúÅÆ¡£\n");
        if (LastDump==""&&Touched)
...
int do_touch(string str,object me)
{
        ...
        {
                command("stare"+query("id", me));
                command("say Õâ°ÑÑ½£¬Äú¾ÍÔÚÒ»±ß¶ùÇÆ×Å°É¡£");
                return 1;
        }
        if (!Playing)
        {
                command("say ¶¼»¹Ã»¿ªÊ¼ÄØ£¬ÄúÃþÅÆ²ØÆðÀ´¸ÉÂïÑ½¡£");
                return 1;
        }
        tell_object(me,"»»Ë­ÃþÅÆ:"+NowPlayerId[NowPlayer]+"\n");
        if( query("id", me) != NowPlayerId[NowPlayer] )
        {
                command("hammer"+query("id", me));
                command("say ÏëÍµÅÆÑ½£¬Ã»ÄÇ÷áÈÝÒ×Å¶¡£");
                return 1;
        }
        if (Touched)
        {
                command("slap"+query("id", me));
                command("say ´òÅÆÀ²£¡±ðÄ¥Ä¥²ä²äµÄ¡£");
                return 1;
        }
        NO_HU=0;
        NO_GANG=0;
...
int do_peng(string str,object me)
{
        ...
        if (!Playing)
        {
                ...
                command("say ¶¼»¹Ã»¿ªÊ¼ÄØ£¬ÅöÄã¸öÍ·°¡¡£");
                return 1;
        }
        if (Touched)
        {
                command("hammer"+query("id", me));
                command("say ±ðÈËÃ»´òÅÆÄú¿ÉÔõ÷áÅöÓ´¡£");
                return 1;
        }
        if( LastDumpId == query("id", me) )
        {
        ...
        if (!str) str=LastDump;
                        ...
                        return notify_fail("ÄÇÕÅ"+do_check_num(str,0)+"ÒÑ¾­¹ýÆÚÁË¡£\n");
        if( query("id", me) == P1Data["Id"])who=P1Data;
        else if( query("id", me) == P2Data["Id"])who=P2Data;
        else if( query("id", me) == P3Data["Id"])who=P3Data;
        else if( query("id", me) == P4Data["Id"])who=P4Data;
        ...
        if (i < 2)
                return notify_fail("ÄãÊÖÖÐÒªÓÐÁ½ÕÅ"+do_check_num(str,0)+"²ÅÄÜÅö¡£\n");

        message_vision("\n$N´ó½Ð£º"+do_check_num(str,0)+"¡«"+HIM+"Ê¯K¡«¡«¡«"+NOR+"\n\n",me);
        for (i=0;i<Play;i++)
        {
                if( NowPlayerId[NowPlayer] == query("id", me))break;
                NowPlayer++;
...
int do_restart(string str)
{
        ...
        message_vision("$NËµµÀ£ºÅÆ¾ÖÖØÐÂÉè¶¨ÇëÓÃ"+HIR+" sit "+NOR+"¼ÓÈëÅÆ¾Ö¡£\n",this_object());
        NowPlayerId[0]="";
        NowPlayerId[1]="";
        NowPlayerId[2]="";
        NowPlayerId[3]="";
...
"f1" : 0,"f2" : 0,"f3" : 0,"f4" : 0,"f5" : 0,"f6" : 0,"f7" : 0,"f8" : 0
        ...
        if (!Play)
        {
                command("say ¶¼Ã»ÈËÍæ¶ù£¬ÒªÎÒ°×Ï´ÅÆÑ½£¿ÃÅ¶ù¶¼Ã»ÓÐ¡£");
                return 1;
        }
        if (Playing)
        {
                command("say ÄÇ¼¸Î»Õý´ò×ÅÄØ¡£");
                return 1;
        }
        command("say ÎÒ°ï×ÅÏ´ÅÆ£¬´ý»á¶ùË­ºúÎÒ¿ÉÒª³ÔºìÅ¶ :)");
        if (Play<4) command("say ¶¼Ã»´ÕÆëËÄ¸ö£¬ÔÛ¾Í²»¼Æ·ÖÁË¡£");
        P1Data["Mj"]="";
        P1Data["OutGang"]="";
        P1Data["Out"]="";
        P1Data["OutPeng"]="";
...
//        for (i=0;i<sizeof(X);i++) printf("%s=%d ",X[i],MjE_Data[X[i]]);
        command("say àÅ£¬ºÃÀ²£¬¿ÉÒÔ¿ªÊ¼ÁË¡£À´°É£¡");
        ...
        i%=Play;
        i++;
        message_vision("["+str+chinese_number(i)+"]ÅÆ¾Ö¿ªÊ¼£º×¯¼Ò"+HIY+NowPlayerId[0]+HIR+countstr+NOR+"ÖÀ³ö÷»×Ó"+ "["+HIG+chinese_number(i=3+random(16))+NOR+ "]µã¡£\n",this_object());
...
int do_setmj(string str)
{
        ...
        if (!str)
        {
                numstr=this_object()->name()+"ÍµÍµ¸æËßÄã£ºÄúÉè¶¨ÁËÕâÐ©¡ª¡ª\n";
                ...
                if (who["AutoTin"]==""&&who["AutoGang"]==""&&who["AutoPeng"]==""&&who["Show"]==""&&who["Auto"]=="") numstr+="ÄãÃ»ÓÐÈÎºÎÉè¶¨¡£\n";
                tell_room(environment(me),this_object()->name()+"´Õ½ü"+me->name()+"¶ú±ßàÖ¹¾ÁË°ëÌì¡£\n",me);
        ...
        if (str=="ting")
        {
        ...
        else if (str=="gang")
                {
                        ...
                        if (check < 3)
                                return notify_fail("ÄãÊÖÖÐÒªÓÐÈýÕÅ"+do_check_num(numstr,0)+"²ÅÄÜÉè¶¨¸Ü´ËÅÆ¡£\n");
                ...
                else if (str=="peng")
                        {
                                ...
                                if (check==0)
                                        return notify_fail("ÄãÃ»"+do_check_num(numstr,0)+"ÕâÕÅÅÆ¡£\n");
                                if (check < 2)
                                        return notify_fail("ÄãÊÖÖÐÒªÓÐÁ½ÕÅ"+do_check_num(numstr,0)+"²ÅÄÜÉè¶¨Åö´ËÅÆ¡£\n");
                        ...
                        else if (str=="show")
                                {
                                ...
                                else if (str=="auto")
                                        {
                                                ...
                                                if (who["AutoTin"]=="")
                                                        return notify_fail("ÇëÏÈÉè¶¨ÒªÌýµÄÅÆ¡£\n");
                                                if (!Touched)
                                                        return notify_fail("ÇëÏÈÃþÅÆÔÙ×ö´ËÉè¶¨¡£\n");
                                                if (numstr=="ting")
                                                        write("ÄãÉè¶¨×Ô¶¯ÌýÅÆ¡£\n");
                                                else return 1;
        ...
        else if (arg=="mj")
                {
                ...
                } else if (arg=="all")
                        {
                        ...
                        } else if (arg=="chicken")
                                {
                                        if (Playing)
                                        {
                                                Mj=LookP+"Á©ÑÛÃÐ³ÉÒ»Ìõ·ì¶ù£¬Õý¾Û¾«»áÉñµØ´òÅÆÄØ¡£\n";
                                                Mj+="»¹Ê£ÏÂ"+sprintf("%d",(end-count-16-Goned)/2)+"ÕÅÅÆ¾ÍÁ÷¾ÖÁË¡£\n";
                                                if (LastDump!="")
                                        ...
                                        else Mj=LookP+"Ò»Ö»Âé½«¼¦ÕýµÈÈË¼ÓÈë(sit)ÅÆ¾Ö¡£";
                                } else if (arg=="end")
                                        {
                                                Mj+="»¹ÓÐ"+chinese_number((end-count-16-Goned)/2)+"ÕÅÅÆ¾ÍÁ÷¾ÖÁË¡£\n";
                                        } else if (arg=="da")
                                                {
                                                        Mj+=LastDumpId+"¸Õ¸Õ´ò³öÁË"+show_mj(LastDump,2)+"¡£\n";
                                                } else return 0;
...
©¦hu    ©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤ ºúÅÆ     ©¦
©¦setmj         Çë¿´×ÀÃæ(look table)         Éè¶¨²ÎÊý ©¦
©¦start ©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤©¤ ÅÆ¾Ö¿ªÊ¼ ©¦
...
¡¡¡¡Ã¿ºú¾ÍÓÐµ×Ì¨£µ£¬ÔÚ¼ÓÉÏËùµÃµÄÌ¨¾ÍÊÇ×ÜÌ¨£¬ÒªÊÇ×ÔÃþ¾Í£ª£³£¬Ïà
µ±ì¶ÎåÒ»µÄËã·¨¡£·ÅÇ¹¾ÍÓÉ×ÜÕ½¼¨ÄÇ¿ÛµôËùÊ§µÄÌ¨Êý¡£ºúÕßÔò¼ÓÉÏÈ¥¡£
