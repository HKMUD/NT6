...
string for_other_msg(object who)
{
        ...
        if(has_guess&&last_player)
        {
                ...
                if(ob)
                {
                        ...
                        if(ob!=who)
                                m=m+"如果你不相信，可以拆穿大话("HIY"dahua"+query("id", ob)+NOR")\n";
                }
        }
        return m;
}
...
int do_guess(string arg)
{
        ...
        if(history)
                history=history+({sprintf("%-20s：%d个%d",query("name", me)+"("+query("id", me)+")",n,p)});
        else
                history=({sprintf("%-20s：%d个%d",query("name", me)+"("+query("id", me)+")",n,p)});
...
int show_all_se(object who)
{
        ...
        m=sprintf("%s%s猜有"HIC"%d"NOR"个%d，",m,query("name", ob),cur_amount,cur_point);
