...
void create()
{
        set("short", "昆仑谷底");
        set("long", @LONG
前面是一堵屏风也似的大山壁。眼前茫茫云海，更无去路，你竟
是置身在一个三面皆空的极高平台上。那平台倒有十余丈方圆，可是
半天临空，上既不得，下又不能，当真是死路一条。这大平台上白皑
皑的都是冰雪，既无树林，更无野兽。
LONG );
...
int do_climb()
{
        ...
        if(n >40)
        {
                message_vision("$N爬了半天，手肘膝盖都已被坚冰割得鲜血淋漓，终于爬了上来。\n", this_player());
                this_player()->move(__DIR__"gudi2");
        }
        else
        {
                message_vision("$N爬了一会，太累了，不小心滑了下来。唉!没办法了，你认命吧。\n", this_player());
