...
int valid_learn(object me)
{
        if( query_temp("weapon", me) || query_temp("secondary_weapon", me) )
                return notify_fail("练灭神拳必须空手。\n");

        if ((int)me->query_skill("force") < 10)
                return notify_fail("你的内功火候不够，无法学灭神拳。\n");

        if( query("max_neili", me)<100 )
                return notify_fail("你的内力太弱，无法练灭神拳。\n");

        if ((int)me->query_skill("unarmed", 1) < 10)
                return notify_fail("你的拳脚根基有限，无法体会灭神拳要诣。\n");

        if ((int)me->query_skill("unarmed", 1) < (int)me->query_skill("mieshenquan", 1))
                return notify_fail("你的拳脚根基火候不足，无法领会更高深的灭神拳。\n");
