...
mixed ask_midao()
{
        ...
        command("whisper"+query("id", me)+"你既然已经入我门来，以"
...
int cost;
   ...
   if( query("family/first", me) ) cost = 250; 
             ...
             if( query("family/gongji", me) < cost ) 
                     return "你为我血刀门效力还不够，等你拥有足够的师门功绩再来兑换血刀使用时间吧。";
        ...
        if (objectp(owner) && owner != this_object())
        {
                ...
                if( query("family/family_name", owner) == "血刀门" )
                        return "血刀现在暂时是你同门师兄"+query("name", owner)+
                               "在用，你要用就去找他吧。";
                else
                        return "本门的血刀现在落在"+query("name", owner)+
                               "手中，你去把他做了，把血刀抢回来！";
        }
...
mixed ask_book()
{
        ...
        if (objectp(owner) && owner != this_object())
        {
                ...
                if( query("family/family_name", owner) == "血刀门" )
                        return "血刀经现在暂时被你同门的"+query("name", owner)+
                               "取去看了，你要看就去找他吧。";
                else
                        return "我血刀门的宝经现在落在了"+query("name", owner)+
                               "手中，你去把它夺回来吧！";
        }
        ob->move(this_object());
