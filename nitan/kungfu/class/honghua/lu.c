...
{
        if( query("family/family_name", ob) != "红花会" && 
        ...
        if( query("family/family_name", ob) == "武当派" )
        {
                command("sigh");
...
mixed ask_skill1()
{
        ...
        if( query("family/family_name", me) != "红花会" && 
           query("family/family_name", me) != "武当派" )
        ...
        if (me->query_skill("furong-jinzhen", 1) < 1)
            ...
            if( query("family/gongji", me)<300 )
                return "嗯，你对"+query("family/family_name", me)+CYN
                       "所作的贡献不够，这招我不传你。";
...
mixed ask_jian()
{
        ...
        if (objectp(owner) && owner != this_object())
        {
                ...
                if( query("family/family_name", owner) == "武当派" )
                        return "我的配剑现在在我武当门人"+query("name", owner)+
                               "手中，你要用就去找他吧。";
                else
                if( query("family/family_name", owner) == "红花会" )
                        return "我的配剑被你们红花会的"+query("name", owner)+
                               "取去了，你要用就去找他吧。";
                else
                        return "我的配剑现在落到了"+query("name", owner)+
                               "手中，你去找他吧！";
        }
        ob->move(this_object());
