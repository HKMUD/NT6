...
int recognize_apprentice(object me)
{
            ...
            !query_temp("learnd_from/鲁班", me) )
        {
                message_vision("鲁班对$N说：好，你既然有心学，我就"
                               "教你一点吧。\n", me);
                set_temp("learnd_from/鲁班", 1, me);
        }

        set_temp("mark/鲁班", 1, me);
        return 1;
}
...
private void luban_say(string msg)
{
        object me;
        me = this_player();
        message("vision","鲁班对"+query("name", me)+"嘀嘀咕咕说了"
                "些话。\n", environment(me), ({ me }));
...
private void user_say(string msg)
{
        ...
        message("vision",query("name", me)+"对鲁班嘀嘀咕咕说了些话。"
                "\n", environment(me), ({ me }));
}
...
int demolish_room(object me)
{
       ...
       command("chat 手里有"+query("name", me)+"的"+
...
KEY;
        ...
        dstfile=replace_string(dstfile,"KEY_NAME",query("name", me)+"家的钥匙");
       ...
       command("give key to "+query("id", me));
        ...
        // 设置玩家的数据库
        set("private_room/build", 1, me);//已经建过房屋
        set("private_room/position", position, me);//房屋所在的位置
        set("private_room/entry", entry, me);//房屋入口处的文件路径
        set("private_room/name", room_name, me);//房屋的名字
        set("private_room/id", room_id, me);//房屋的ID
        set("private_room/type", room_type, me);//房屋的类型
        set("private_room/key_no", 1, me);//使用原配钥匙
        me->save();
