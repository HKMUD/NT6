...
void lost()
{
        ...
        command("chat恭喜"+query("name", ob)+"被推举为当今罚恶使者！\n");
        set("fighting", 1, me);
        remove_call_out("do_copy");
        call_out("do_copy", 1, me, ob);
...
int do_copy(object me, object ob)
{
        ...
        set("party/rank", HIW+"罚恶使者"+NOR, ob);

        set("short", HIW+"第"+chinese_number(query("generation", me))+"代罚恶使者"+NOR+""+query("name", me)+"(Faeshizhe)", me);
        delete("title", me);
