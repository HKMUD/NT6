...
/*
        message("member", HIM "\n【泥潭会员】" + me->query_idname() + HIM " 领取二小时的高效 饭κ奔淇技剖薄n" NOR,
                all_interactive(), 0);
*/
...
mixed ask_duanwu()
{
	...
	if (query_temp("duanwu_time", me) > time())
		tell_object(me, HIY "南贤说道：" + query("name", me)  + "你刚刚已经拿过了，再拿需要等待10秒，每个人每天只能吃一次哟！\n"NOR);
	else {
		tell_object(me, HIY "南贤说道：" + query("name", me)  + "端午节快乐！\n"NOR);
 		message_vision(HIY "南贤从怀里拿出一个粽子送给了 " + query("name", me) + "！\n" NOR);
...
mixed ask_buchang()
{
        ...
        me->save();
        tell_object(me, HIY "\n你领取了(" HIM "当机补偿" NOR + HIY ")，祝你在王者之战里玩的愉快！\n" NOR);
        return 1;
}
...
mixed ask_reborn()
{
        ...
        {
                message_vision("$N对$n一拱手，道： 阋丫缆只亓耍铱淳筒挥昧税桑n",
                               this_object(), who);
                return 1;
        }
