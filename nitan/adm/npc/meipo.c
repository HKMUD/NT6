...
void create()
{
        ...
        set("gender", "女性" );
        //set("nickname", HIR "结婚得有始有终，" HIC "必须" HIR "走完流程！！！！！！" NOR); 
        set("title", "红娘庄");
        set("age", 43);
        set("long","一位精明能干的老媒婆。\n");
...
int accept_object(object me, object ob)//交钱
{
        ...
        log_file("static/MARRY",sprintf("%s(%s)结婚交钱in%s。\n",query("name", me),
...
/*
        ...
        if( (query("age", me)<14) && (query("gender", me) == "男性" )
                 || (query("age", me)<14) && (query("gender", me) == "无性" )
...
*/
      ...
      if( query("couple/married", obj)>2 )
        ...
        {
                if( query("gender", obj) == "女性" || (query("gender", obj) == "无性" && query("gender", me) == "男性") )
                        message_vision(MAG "\n$N对著$n问道：“你愿意嫁给我吗？”\n\n"NOR, me,obj);
                else
                        message_vision(MAG "\n$N对著$n问道：“你愿意娶我吗？”\n\n"NOR, me,obj);
...
{
        delete_temp("wait_time", ob);
        delete_temp("marry", ob);//取消一切有关结婚的参数
        delete("marry", ob);
