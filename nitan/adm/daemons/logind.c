...
nosave string *banned_name = ({
        ...
        "胡锦涛", "【","】","马克思","恩格斯","列宁","抄","手淫","老百姓","临时人物","女马","大便",
});
...
void logon(object ob)
{
        ...
        if( query_temp("tomud", ob) )
                write(ALERT("经典文字MUD游戏『王者之战』，群号『46899981』，本游戏为免费游戏！$$" +
        ...
        write(sprintf("目前共有 " HIY "%d" NOR " 位神仙、" HIW "%d" NOR " 位江湖人士在江湖中，以及 " HIB "%d" NOR " 位朋友正在步入江湖的途中。"+ ((wiz_lock_level>0)?HIR"(禁止玩家登入)":HIC"(允许玩家登入)") + "\n\n" NOR,
...
LONG ;
                ...
                //MAIL_D->queue_mail(0, 0, arg, "王者之战 - " + query("id", ob) + " 角色密码", msg);
                SMTP_D->send_passmail(0, 0, arg, "王者之战 - " + query("id", ob) + " 角色密码", msg);
        } else
                write("您输入的资料与该角色不符。\n");
...
#ifdef NO_FEE
        if( query_temp("tomud", ob) )
                write(ALERT("文字MUD游戏『王者之战』将永久免费开放！请新老玩家相互转告！\n"));
#else
        /*
