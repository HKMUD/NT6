...
定身        降低对方闪避30%
迟缓        降低对方攻击速度的50%
混乱        在一定回合内攻击随机目标，包括己方队友
*/
// 神佑 战斗时当HP=0时，20%的几率出现神佑复活效果，复活以后拥有少量HP
...
mapping value_2_props = ([
        ...
        "per"           : "容貌",       // rings              a
        "gold_find"     : "获取黄金几率",     // weapon/rings a
...
mapping value_3_props = ([        
        "attribute"     : "所有属性",   // rings              b
        "max_neili"     : "内力最大值", // weapon/armor/rings b
        "max_qi"        : "气血最大值", // weapon/armor/rings b
        "ref_neili"     : "内力恢复",   // armor/rings        b
        "ref_qi"        : "生命恢复",   // armor/rings        b
        "ref_jing"      : "精气恢复",   // armor/rings        b
        ...
        "add_pot"       : "获取额外潜能",       // weapon/rings   b
        "add_mar"       : "获取额外体会",       // weapon/rings   b
]);    
...
mapping value_5_props = ([             
        ...
        "suck_neili"    : "偷取内力",           // weapon 命中偷取内力几率：命中敌人时按伤害值的10%率回复内力   c
        "suck_qi"       : "偷取生命",           // weapon 命中偷取生命几率：命中敌人时按伤害值的10%比率回复生命 c
        "double_damage" : "双倍伤害",           // weapon/rings 增加杀气：加一点杀气，暴击（双倍伤害）的概率增加1%  c
        "through_armor" : "破甲",               // weapon/rings 破甲-攻击时无视目标防具的保护力through_armor        c        
        "qi_vs_neili"   : "受损的生命转为内力", // rings 30%的机会受损的生命x%转换为内力  c   
        "absorb_blood"  : "天魔附体",           // weapon/rings 提高自身战斗力30%         c

        "add_reward"    : "获取额外奖励",       // weapon/rings   c
        "add_exp"       : "获取额外经验",       // weapon/rings   c
        "add_force"     : "获取额外先天真气",   // weapon/rings   c        
        "add_blind"     : "致盲",               // weapon 终极兵器 致盲-让对手失明 c
        "avoid_blind"   : "忽视致盲",           // armor/rings 终极防具            c
]);

mapping value_10_props = ([          
        "clear_force"   : "破气",                       // weapon 破除对手所有内力，让对手内力为0 d       
        "add_freeze"    : "冰冻",                       // weapon 终极兵器 冰冻-迟缓，减缓对方攻击，攻击pfm速度降低，无任何防御，无躲闪，无招架，就是挨打 d
        "add_dizziness" : "眩晕",                       // weapon 终极兵器 眩晕，不能进行任何行动，受到攻击会醒来，否则5秒内一直不动，解除战斗状态 d
        ...
        "avoid_dizziness": "忽视眩晕",                  // armor/rings 终极防具 d
        ...
        "fatal_blow"    : "必杀率",                     // weapon 终极兵器 必杀率-拥有20%的几率损伤目标的生命x%     d                                    d
        "slaughter"     : "杀戮",                       // weapon 杀戮-攻击时有几率对当前房间所有敌人进行一次攻击   d  
...
mapping all_props = ([
        ...
        "suck_neili"    : "偷取内力",   // weapon 命中偷取内力几率：命中敌人时按伤害值的10%率回复内力
        "suck_qi"       : "偷取生命",   // weapon 命中偷取生命几率：命中敌人时按伤害值的10%比率回复生命
        ...
        "clear_force"   : "破气",               // weapon
        
        //"block_heal"    : "防止对手自疗",       // weapon
        "absorb_blood"  : "天魔附体",           // weapon/rings 提高自身战斗力30%
        ...
        "add_dizziness" : "眩晕",                       // weapon 终极兵器 眩晕，不能进行任何行动，受到攻击会醒来，否则5秒内一直不动，解除战斗状态
        ...
        "fatal_blow"    : "必杀率",                     // weapon 终极兵器 必杀率-拥有20%的几率损伤目标的生命x%     
        "anti_fatal"    : "忽视必杀",                   // armor/rings
        ...
        "full_self"     : "战神附体",                   // armor 终极防具 战神附体-气血内力自动恢复
        "avoid_die"     : "浴血重生",                   // armor/ring 凤凰涅槃，浴血重生，忽视死亡
...
mixed weapon_props = ({
        ...
        ({ "research_effect", "practice_effect", "study_effect", "learn_effect", "derive_effect", "damage1",
           ...
           "suck_neili", "avoid_force", "add_exp", "add_pot", "add_mar", "add_force", "through_armor", "skill", // skill特殊处理 
        }), 
...
/*
                                        if( query("quality_level", ob) == 3 ) // 珍稀
                                                CHANNEL_D->channel_broadcast("news",  
                                                        killer->name() + "杀死" + victim->name() + NOR YEL"，从其身上掉出一"+query("unit", ob)+HIR" 珍"NOR RED"稀"HIW"级 "NOR YEL+ob->name()+YEL"。\n"NOR);
                                        else if( query("quality_level", ob) == 4 ) // 史诗
*/
                                        if( query("quality_level", ob) == 4 ) // 史诗
                                                CHANNEL_D->channel_broadcast("news",  
                                                        killer->name() + "杀死" + victim->name() + NOR YEL"，从其身上掉出一"+query("unit", ob)+HIM" 史"NOR MAG"诗"HIW"级 "NOR YEL+ob->name()+YEL"。\n"NOR);
                                        else if( query("quality_level", ob) == 5 ) // 传说
                                                CHANNEL_D->channel_broadcast("news",  
                                                        killer->name() + "杀死" + victim->name() + NOR YEL"，从其身上掉出一"+query("unit", ob)+HIY" 传"NOR YEL"说"HIW"级 "NOR YEL+ob->name()+YEL"。\n"NOR);
                                        else if( query("quality_level", ob) == 6 ) // 神器
                                        {
                                                CHANNEL_D->channel_broadcast("news",  
                                                        ...
                                                        killer->name() + "杀死" + victim->name() + NOR YEL"，从其身上掉出一"+query("unit", ob)+" "+HBRED+HIW"神器"NOR+HIW"级 "NOR YEL+ob->name()+YEL"。\n"NOR);
