...
int hide_anywhere(object me, object item)
{
        if( item->item_owner() != query("id", me) &&
            query("item_owner", item) != query("id", me) && query("owner", item) != query("id", me)) // 方便掉落物品拥有hide能力
                return 0;
...
int do_imbue(object me, object item, object imbue)
{
        ...
        if( query("magic/imbue_element", item) )
        {
                if( !query("magic/element", imbue) )
                        return notify_fail(item->name() + "必须首先浸入具有五行属性的物品。\n");
        }
        else
        {
...
int do_enchase(object me, object item, object tessera)
{
        ...
        if( !item->is_weapon() && !item->is_unarmed_weapon() && query("can_be_enchased", tessera) )
                return notify_fail(tessera->name() + "只能镶嵌在兵器类装备上。\n");
        */
        ...
        if( query("magic/type", item) && query("magic/type", tessera) && query("magic/type", tessera) != query("magic/type", item) )
                return notify_fail(item->name() + "上已经拥有五行灵力属性和镶嵌物的五行灵力属性不一致，无法镶嵌融合。\n");
        ...
        if( !query("magic/type", item) && (item->is_weapon() || item->is_unarmed_weapon()))
        {
                ...
                if( query("enchase/used", item)+1 >= query("enchase/flute", item) )
                        return notify_fail(item->name() + "上的剩下的唯一的凹槽是用来镶嵌具有灵力的物品。\n");
        }

        temp = query("enchase", tessera);
        if( undefinedp(temp["type"]) )
                return notify_fail(tessera->name() + "的魔力属性不适合镶嵌在" + item->name() + "上。\n");
        else
        {
                if (temp["type"] != "all" &&
        ...
        if( !tessera->is_rune() && !query("can_be_enchased", tessera) && temp["level"] != query("require/level", item) )
                return notify_fail(tessera->name() + "的等级不适合镶嵌在" + item->name() + "上。\n");
        ...
        /*
        if (tessera->is_rune())
                data=query("enchase/rune_prop", item);//符文的魔法能力单独标记
        else
        */
...
int do_forging(object me, object item, object ob)
{
        ...
        if( (int)me->query_skill("forging", 1) < 200 )
                return notify_fail("你对锻造技艺掌握不够。\n");

        if( query("level", me)/10*10 <= query("require/level", item) )
                return notify_fail("你目前的能力无法再继续锻造你的装备。\n");
        ...
        message_sort(HIM "$N" HIM "手握" +ob->name()+ HIM "，一股内力丝丝的传了进去，" +ob->name()+
                     HIM "忽然间融化在掌心，晶莹欲透！$N"
                     HIM "随手一挥，将一汪清液洒在$n"
                     HIM "上，" HIM "$n" HIM "犹如烈火中重生一般，散发出绚目的光彩！\n" NOR,
                     me, item);
        ...
        if( n >= level+10 )
        {
                ...
                //set("enchase/steady", 0, item);   // 强化次数恢复0
                //set("enchase/increase", 0, item); // 改造次数恢复0
        }
...
int do_require(object me, object ob)
{
        ...
        set("require", temp, ob);
        message_vision(HIY +ob->name()+ HIY "隐隐的显过了一道奇异的光芒，随即变得平静。\n" NOR, me);
        ob->save();
}
...
void do_enchase_attack(object item, object me, object victim, int damage)
{
        ...
        for( i=0;i<n;i++ )
        {
                ...
                case "add_freeze":
                        ...
                        message_combatd(msg, me, victim);
                        set_temp("freeze", 1, victim); // combatd和perform中调用
                        victim->start_call_out((: call_other, __FILE__, "remove_effect", victim, "冰冻" :), 5);
                        break;
                
                case "add_burning":
                        extra = enchase[apply[i]]; // 灼伤
                        if( random(100) > extra ) break;
                        avoid = query_temp("apply/avoid_burning", victim);
                        if( random(100) < avoid ) break;
                        
                        msg = HIR "$n" HIR "只觉自己被置于火山之中，身体开始自燃，气血和内力开始不停的下降。\n" NOR;
                        message_combatd(msg, me, victim);
                ...
                case "add_dizziness":
                        ...
                        extra = enchase[apply[i]]; // 眩晕
                        if( random(100) > extra ) break;
                        avoid = query_temp("apply/avoid_dizziness", victim);
                        if( random(100) < avoid ) break;
                        
                        msg = HIY "$n" HIY "只觉头脑昏昏沉沉，昏然欲睡，精神不振，迷迷糊糊，处于眩晕状态中。\n" NOR;
                        message_combatd(msg, me, victim);
                        victim->remove_all_killer();
                        all_inventory(environment(victim))->remove_killer(victim);
                        set_temp("dizziness", 1, victim);
                        set_temp("override/receive_damage", (: call_other, __FILE__, "receive_damage" :), victim); // damage调用恢复
                        victim->start_call_out((: call_other, __FILE__, "remove_effect", victim, "眩晕" :), 5);
                        break;
 
                 case "add_forget":
                        extra = enchase[apply[i]]; // 遗忘
                        if( random(100) > extra ) break;
                        avoid = query_temp("apply/avoid_forget", victim);
                        if( random(100) < avoid ) break;
                        
                        msg = HIR "$n" HIR "突然心中一乱，大脑一片空白，顿时遗忘了绝招如何使用。\n" NOR;
                        message_combatd(msg, me, victim);
                        set_temp("forget", 1, victim); 
                        victim->start_call_out((: call_other, __FILE__, "remove_effect", victim, "遗忘" :), 5);
                        break;
                /*
                case "add_weak":
                        extra = enchase[apply[i]]; // 虚弱
                        if( random(100) > extra ) break;
                        avoid = query_temp("apply/avoid_weak", victim);
                        if( random(100) < avoid ) break;
                        
                        msg = HIR "$n" HIR "顿时只觉得身体非常的虚弱，每个动作都非常困难，一点力气也使不出来了。。。\n" NOR;
                        message_combatd(msg, me, victim);
                        victim->set_weak(5);
                        break;

                case "add_busy":
                        extra = enchase[apply[i]]; // 忙乱
                        if( random(100) > extra ) break;
                        avoid = query_temp("apply/avoid_busy", victim);
                        if( random(100) < avoid ) break;
                        
                        msg = HIR "「" + item->name() + HIR "」追加「" + victim->name() + HIY + "」忙乱效果：" + sprintf("%d", 5) + "秒\n" NOR;
                        tell_object(me, msg);
                ...
                case "add_damage":
                        ...
                        msg = HIR "「" + item->name() + HIR "」追加「" + victim->name() + HIY + "」伤害：" + sprintf("%d", extra) + "点\n" NOR;
                        tell_object(me, msg);
                        victim->receive_damage("qi", extra, me);
                        break;
                ...
                case "no_exert":
                        extra = enchase[apply[i]];                       
                        msg = HIR "「" + item->name() + HIR "」追加「" + victim->name() + HIY + "」内息紊乱：" + sprintf("%d", extra) + "秒\n" NOR;
                        tell_object(me, msg);
                        if( !query_temp("no_exert", victim) ) 
                                set_temp("no_exert", 1, victim);
                        tell_object(victim, HIM "你被" + me->name() + HIM "的" + HIY + item->name() + HIM "打中，顿觉内息紊乱！\n" NOR);

                        // 消除效果
                        call_out("remove_effect", extra, victim, "内息紊乱");
                        break;

                case "no_perform":
                        extra = enchase[apply[i]]; 
                        msg = HIR "「" + item->name() + HIR "」追加「" + victim->name() + HIY + "」力道涣散：" + sprintf("%d", extra) + "秒\n" NOR;
                        tell_object(me, msg);
                        if( !query_temp("no_perform", victim) ) 
                                set_temp("no_perform", 1, victim);
                        tell_object(victim, HIM "你被" + me->name() + HIM "的" + HIY + item->name() + HIM "打中，顿觉力道涣散！\n" NOR);

                        // 消除效果
                        call_out("remove_effect", extra, "力道涣散");
                        break;
                */
                case "clear_force":
                        extra = enchase[apply[i]]; 
                        ...
                        msg = HIR "「" + item->name() + HIR "」追加「" + victim->name() + HIY + "」破除气海要穴。\n" NOR;
                        tell_object(me, msg);
                        set("neili", 0, victim);
                        tell_object(victim, HIM "你被" + me->name() + HIM "的" + HIY + item->name() + HIM "打中气海要穴，顿觉内力涣散！\n" NOR);
                        break;
                ...
                case "reduce_combat":
                        ...
                        msg = HIR "「" + item->name() + HIR "」追加「$n" + HIR "」削弱战力：削弱攻、防、回避、命中各30%。\n" NOR;
                        message_combatd(msg, me, victim);
                        ...
                        victim->start_call_out((: call_other, __FILE__, "remove_effect", victim, "消弱战力", a1, a2, a3, a4 :), 15);
                        break;
                ...
                case "fatal_blow":
                        ...
                        msg = HIR "「" + item->name() + HIR "」追加「$n" + HIY + "」致命一击！伤害：" + sprintf("%d", extra) + "点\n" NOR;
                        message_combatd(msg, me, victim);
                        victim->receive_damage("qi", extra, me);
                ...
                case "absorb_blood":
                        ...
                        msg = HIG "「" + item->name() + HIG "」光芒万丈，天魔伴随着光芒从天而降对$N" + HIG "进行附体。\n" NOR;
                        ...
                        me->start_call_out((: call_other, __FILE__, "remove_effect", me, "天魔附体", a1, a2, a3, a4 :), 15);
                        break;
                ...
                case "slaughter":
                        ...
                        msg = HIR "「" + item->name() + HIR "」杀气漫天，一条条杀气伴随着闪电从天而降。\n" NOR;
                        message_combatd(msg, me);
...
varargs int receive_damage(object me, string type, int damage, object who)
{
        ...
        tell_object(me, HIR "由于受到攻击，你眩晕状态解除，恢复了正常。\n" NOR);
        return 0;
}
