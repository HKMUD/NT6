...
int dispersion( object me )
{
    ...
    CHANNEL_D->do_channel(me,"sys",sprintf("%s手动执行幻境游戏主控中心派发随机物件函数(%d个物件)。",query("id", me),times));
...
int del_all( object me )
{
    ...
    tell_object( me, "Ok. 已执行函数，请留意 sys 信息。\n");
    CHANNEL_D->do_channel(me,"sys",sprintf("%s手动执行幻境游戏主控中心的强制清除所有境内物件函数。",query("id", me)));
    clean_hj();
    return 1;
}
