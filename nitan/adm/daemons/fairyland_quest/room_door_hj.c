...
int do_enter_hj(string arg)
{
    ...
    tell_room(environment(me),query("name", me)+"使劲地往上一跳，突然卷来一阵旋风，"+query("name", me)+"顿时被风刮得不知所踪。\n",me);
    me->move(rooms);
    write("你使劲地往上一跳，忽地平地卷起一阵旋风，恍惚中你已来到了「幻境」。\n");
    tell_room(rooms,"忽然一阵旋风袭过，"+query("name", me)+"已出现在这里。\n",me);
    return 1;
}
*/
