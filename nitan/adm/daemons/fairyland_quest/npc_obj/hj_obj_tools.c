...
void delete_me()
{
    if( query("real_name") )
    {
        message_vision(query("real_name", this_object())+"灵气已尽，「嘭」地一声消失不见了。\n",this_object());
        delete("qzzq_now", environment());
    }
    else
        message_vision(query("name", this_object())+"灵气已尽，「嘭」地一声消失不见了。\n",this_object());
...
int do_fadai(string arg)
{
    ...
    if(arg != "all")
    {
        ...
        if( !query_temp("hj_hp", target) || query_temp("hj_hp", target)<1 )
            return notify_fail(query("name", target)+"没有幻境里的气息，无法对其进行攻击。\n");
        if(!living(target))
    ...
    for(i=0;i<sizeof(inv_here);i++)
    {
        ...
        if(srl)
        {
            message_vision( HIC+query("real_name")+""HIC"发出一阵耀眼光芒！……\n"
                +"却见$N"HIC"身上的"+query("name", srl)+HIC
...
int do_xunren(string arg)
{
    ...
    if( arg == query("id", me) )
        return notify_fail("幻境里虽然有些妖魔鬼怪，但你"+query("name", me)+"还是一个而已，没有更多的了。\n");
...
int do_kuitan(string arg)
{
    ...
    if( !query_temp("hj_hp", target) || query_temp("hj_hp", target)<1 || 
       query_temp("huanjing", target) != "start" )
        return notify_fail(query("name", target)+"已没有丝毫幻境里的气息，算了吧。\n");
    if(!living(target))
