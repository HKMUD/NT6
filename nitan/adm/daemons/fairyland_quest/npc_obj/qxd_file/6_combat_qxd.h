...
void fighting()
{
    ...
    {
        message_vision(CYN"$N"NOR+CYN"身上的"+query("name", srl)+NOR+CYN"突然发出一阵光芒！\n"NOR,target);
...
//            if( !use_tools ) use_tools = present( fight_use, environment( me ) );
    ...
    if( figh_type == "hand" )
    {
        ...
        tell_room(environment(me),CYN"\n"+query("name", me)+NOR+CYN"奋力向"+query("name", target)+NOR+CYN+temp+"！\n"NOR,({me,target}));
        tell_object(target,CYN"\n"+query("name", me)+NOR+CYN"奋力向你"+temp+"，造成了"HIR""+damages+""NOR+CYN"点的伤害！\n"NOR);
        tell_object(me,CYN"\n你奋力向"+query("name", target)+NOR+CYN+temp+"，造成了"HIG""+damages+""NOR+CYN"点的伤害！\n");
    ...
    if( figh_type == "tools" )
    {
        ...
        if( query("hj_game/obj", use_tools) == "sword" )
        {
            ...
            tell_object(target,CYN"\n"+query("name", me)+NOR+CYN"握起"+query("name", use_tools)+NOR+CYN"向你"+temp+"来，造成了"HIR+damages+NOR+CYN"点的伤害！\n"NOR);
            tell_object(me,CYN"\n你握起"+query("name", use_tools)+NOR+CYN"向"+query("name", target)+NOR+CYN+temp+"去，造成了"HIG+damages+NOR+CYN"点的伤害！\n");
        }
        // 否则是魔法水晶类
        else
        {
            ...
            tell_room(environment(me),color_name+"只见"+query("att_msg", use_tools)+"向"+query("name", target)+NOR+color_name+"攻去，造成了"HIG+damages+NOR+color_name+"点的伤害！\n"NOR,target);
            tell_object(target,color_name+"只见"+query("att_msg", use_tools)+"向你攻来，造成了"HIR+damages+NOR+color_name+"点的伤害！\n"NOR);
        }
    }
    ...
    if( figh_type == "skills" )
    {
        ...
        tell_room(environment(me),color_name+"\n"+query("name", me)+NOR+color_name+"运起「"+skills_name+"」向"+query("name", target)+NOR+color_name+"攻去！\n"NOR,({me,target}));
        tell_object(target,color_name+"\n"+query("name", me)+NOR+color_name+"运起「"+skills_name+"」向你攻来，造成了"HIR+damages+NOR+color_name+"点的伤害！\n"NOR);
        tell_object(me,color_name+"\n你运起「"+skills_name+"」向"+query("name", target)+NOR+color_name+"攻去，造成了"HIG+damages+NOR+color_name+"点的伤害！\n"NOR);
