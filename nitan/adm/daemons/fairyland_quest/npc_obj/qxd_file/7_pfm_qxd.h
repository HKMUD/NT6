    ...
    */
    if( pfm_id == "dh" )
    {
        ...
        else
        {
            ...
            tell_object(me,HIR+query("name", target)+HIR"被拍个正中，突然两眼发直，整个人呆住了似的。(造成忙时"+target_busy+"秒)\n\n"NOR);
            tell_room(environment(target),HIR+query("name", target)+HIR"被拍个正中，突然两眼发直，整个人呆住了似的。\n\n"NOR,({me,target}));
