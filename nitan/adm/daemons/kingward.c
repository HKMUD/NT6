...
nosave string battle_name = HIY"国家的荣耀"NOR;
...
int start_kingwar()
{
        ...
        set_temp("warquest/quest", "挥军北上，抵抗入侵", marshal);
...
void start_battle()
{
        ...
        if( sizeof(battle_player) < 6 )
        {
                ...
                battle_open_flag = 0;
                battle_start_time = 0;
                CHANNEL_D->channel_broadcast("war", "报名参与「"+battle_name+HIR"」战役的玩家不足 6 人，取消本次战役。");
                return;
        }
...
void start_join()
{
        CHANNEL_D->channel_broadcast("war", HIY"荣耀"NOR YEL"之战"HIR"发出战争集结，即将在 "+(BATTLE_JOIN_TIME/60)+
                                            " 分钟后发动「"+battle_name+HIR"」，请使用 war join 指令加入战役。");
                                ...
                                if (file == "/maze/battle3/eastgate1")
                                {
                                        ...
                                        if( 1 )
                                        {
                                                ...
                                                case "enlist" :
                                                        ...
                                                        break;
                                                case "forage" :
                                                        message_vision(HIY "$N" HIY "率领的解粮大军终于到达了襄阳城，" +
...
void calculate_score(int flag)
{
        ...
        if (arrayp(total) && sizeof(total) > 0)
        {
                for (i = 0; i < sizeof(total); i++)
                {
                        ...
                        tell_object(total[i], HIR "大宋军队撤兵了，准备班师回朝吧！\n" NOR);
                       ...
                       if( reward >= 2000 && time()-query_temp("warquest/time", total[i]) >= 600 )
                        ...
                        tell_object(total[i], HIW "因抗宋有功，你获得了 " + HIR + chinese_number(jungong) +
                                              HIW " 策军功！！\n" NOR);
                }
        }
        battle_open_flag = 0;
