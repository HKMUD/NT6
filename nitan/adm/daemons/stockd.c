...
void parse_data(string data)
{
        ...
        if( sizeof(data) )
        {
                ...
                if( pos_s == -1 )
                {
                        CHANNEL_D->channel_broadcast("sys", "无法解析沪深A股个股资料");
                        return;
                }
                ...
                while( sscanf(temp, "%*svar hq_str_%s=\"%s\";\n%s",code, str, temp) >= 3 )
                {
                        ...
                        stocks[code]["股票名称"] = stock_data[0];
                        stocks[code]["昨收盘"] = to_float(stock_data[2]);
                        stocks[code]["现价"] = to_float(stock_data[3]);
                        stocks[code]["开盘"] = to_float(stock_data[1]);
                        stocks[code]["状态"] = status;

                        stocks[code]["涨跌"] = to_float(stock_data[3])-to_float(stock_data[2]);
                        stocks[code]["涨跌幅"] = (to_float(stock_data[3])-to_float(stock_data[2]))*100/to_float(stock_data[2]);
                        stocks[code]["最高"] = to_float(stock_data[4]);
                        stocks[code]["最低"] = to_float(stock_data[5]);
                        stocks[code]["成交张数"] = to_int(stock_data[8]);
                        stocks[code]["日期"] = stock_data[30];
                        stocks[code]["时间"] = stock_data[31];

#ifdef DEBUG
                        CHANNEL_D->channel_broadcast("nch", "成功解析 "+code+stocks[code]["股票名称"]+" 个股资料(现价 "+stocks[code]["现价"]+")");
#endif
                }
        }
...
void get_stock_data()
{
        ...
        if( fd < 0 )
        {
                CHANNEL_D->channel_broadcast("nch", "无法与新浪股票服务器建立连接。\n");
                return;
        }
        ...
        if( err != EESUCCESS )
                CHANNEL_D->channel_broadcast("nch", "无法连线股票网页。");

#ifdef DEBUG               
        CHANNEL_D->channel_broadcast("nch", "与新浪股票服务器建立连接成功！");
#endif
        return;
}
...
void refresh_stock_data()
{
...
#ifndef DEBUG
        if( nowtime[2] < 1 || nowtime[2] > 5 ) // 周末休盘
                return;

        if( nowtime[1] < 9
        ||  (nowtime[1] == 9 && nowtime[0] < 15) ) // 早上未开
                return;

        if( nowtime[1] == 11 && nowtime[0] > 45
        ||  (nowtime[1] == 12 && nowtime[0] < 45) ) // 中午休盘
                return;

        if( nowtime[1] > 15
        ||  (nowtime[1] == 15 && nowtime[0] > 15) ) // 下午休盘
                return;
#endif
        ...
        CHANNEL_D->channel_broadcast("sys", "开始更新股市资料。");
        get_stock_data();
}
...
void write_callback(int fd)
{
        ...
        CHANNEL_D->channel_broadcast("nch", "开始抓取股票资料。");
