...
void create()
{
        ...
        set_name(HIY "炎黄Ⅱ中秋月饼" NOR, ({ "yh2 yuebing2", "yh2", "yuebing2" }) );
        set_weight(1);
        if (clonep())
...
int do_effect(object me)
{
        ...
        if(! objectp(present("yh2 yuebing2", me)))return 0;
               ...
               tell_object(me, HIY "你哈哈一笑，将整个炎黄Ⅱ中秋月饼一起吞了。\n" NOR, me);
        ...
        if (me->query("combat_exp") > 5000)
                tell_object(me, HIM "你获得了五十万点经验和一百万点潜能以及" + chinese_number(tihui) + "点实战体会。\n" NOR, me);
                
        if (MEMBER_D->is_valib_member(me->query("id")))
                tell_object(me, HIM "你获得了五万炎黄币(yhb)！\n" NOR, me);
