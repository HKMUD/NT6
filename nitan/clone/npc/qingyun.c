...
void give_place(object ob)
{
        ...
        while (sizeof(files)>0)
        {
                ...
                n1 = shorts[0];
                shorts -= ({ n1 });
                tell_object(ob,this_object()->name()+"说道：从"HIW+to_chinese(dir)+NOR"的"HIW+query("short", room)+NOR+
...
string cancel_job()
{
        ...
        if( !query("quest/kill", who))return "你没有领任务，有什么好取消的";
