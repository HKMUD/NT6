...
int stop_auction()
{
        ...
        } else            
        {
                ...
                } else
                {
                        ...
                        shout(HIY"【行商】"NOR+WHT+operator->name()+WHT"["+query("id", operator)+WHT"]：恭喜"+the_owner->name()+NOR+WHT"获得了"+now_shop+NOR+WHT"店铺的所有权。\n"NOR);
...
int finish_auction(string arg)
{
        ...
        shout(HIY"【行商】"NOR+WHT+operator->name()+WHT"["+query("id", operator)+WHT"]：拍卖大会到此结束，各位可用("HIY"shop"NOR+WHT")查看各家店铺详情。\n"NOR);
...
int do_autoauction()
{
        ...
        CHANNEL_D->do_channel(ob,"sys",query("id", me)+"将拍卖进程转入自动模式！");
        set("operator", "situ heng");
        save();
