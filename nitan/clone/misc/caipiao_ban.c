...
void check(object me)
{
 ...
 /*query_temp("check", if( oldcrypt(me),yn) != yn){
        ...
        case "二等奖":money=query("last_gold")/10;
            break;
        case "三等奖":money=query("last_gold")/100;
            break;
        case "四等奖":money=query("last_gold")/1000;
            break;
        }
        for(i=0;i<money;i++)
        {
        ...
        write(HIR"你中了"+chinese_number(money)+"两黄金,已经拨入你的账户！\n");
        message("system",HIW"〖泥潭福彩〗："+HIC" "+query("name",me)+HIW" 中了"+HIY" "+chinese_number(money)+HIW" 两黄金，已经拨入帐户！\n"NOR,users());
...
int do_read(string arg)
{
    ...
    if(arg == "duijiang")
    {
        ...
        write("三等奖：共有四个数字吻合        ——全部累计奖金/100.\n");
        write("四等奖：仅有三个数字吻合        ——全部累计奖金/1000.\n");
        write("祝您玩得痛快!\n");
        return 1;
    }
    else        write("你要看什么？购买规则请read rules，购买情况请read biao,兑奖规则请read duijiang。\n");
    return 1;
}
string show_player()
{
    ...
    if(!query("duijiang"))
    {
        ...
        while (i<h)
        {
            ...
            /*if (strlen(name["name"])==6)
                str+= sprintf("┃%-28s┃%22s┃%-6s┃\n",HIR+name["name"]+NOR,(int)name["z_num"],name["jieguo"],);//删除name["jieguo"]前(int)name["t_num"]  by wenwu
        ...
        str+=HIW"本期彩票累计奖金："HIY+query("all_gold")+HIG" 两黄金。\n"NOR;
    ...
    str+=HIW"下期彩票预计奖金："HIY+query("all_gold")+HIW" 两黄金。\n"NOR;
    return "/feature/more.c"->start_more(str);
}
...
void auto_post(string title,string body)
{
    ...
    note["title"] = title;
    note["author"] ="彩票板";
   // "/cmds/std/news.c"->done_post(this_object(),note,body);暂注释掉新闻提醒领奖 by wenwu
    return;
}
void clean_data()
{
